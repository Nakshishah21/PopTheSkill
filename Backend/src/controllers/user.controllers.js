...
export const discoverUsers = asyncHandler(async (req, res) => {
  ...
  const computeAverageRating = async (username) => {
    ...
    return {
      avgRating: result?.avgRating ? Number(result.avgRating.toFixed(2)) : 0,
      count: result?.count || 0,
    };
  };

  // Build lists of users for different sections
  const forYouCandidates = [];
  const webDevCandidates = [];
  const mlCandidates = [];
  const otherCandidates = [];

  users.forEach((user) => {
    const userSkills = Array.isArray(user.skillsProficientAt) ? user.skillsProficientAt : [];
    const userWantsToLearn = Array.isArray(req.user.skillsToLearn) ? req.user.skillsToLearn : [];

    const skillMatches = userSkills.filter((skill) => userWantsToLearn.includes(skill)).length;
    const hasWebDevSkill = userSkills.some((skill) => webDevSkills.includes(skill));
    const hasMlSkill = userSkills.some((skill) => machineLearningSkills.includes(skill));

    if (skillMatches > 0) {
      forYouCandidates.push({ user, skillMatches });
    } else if (hasWebDevSkill) {
      webDevCandidates.push(user);
    } else if (hasMlSkill) {
      mlCandidates.push(user);
    } else {
      otherCandidates.push(user);
    }
  });

  // Compute ratings and sort recommendations
  const enrichWithRating = async (items) => {
    const enriched = await Promise.all(
      items.map(async (item) => {
        const username = item.user ? item.user.username : item.username;
        const { avgRating } = await computeAverageRating(username);
  ...
  const forYouWithRatings = await enrichWithRating(forYouCandidates);
  // Sort by a combined score (skill match first, then rating)
  forYouWithRatings.sort((a, b) => {
    if ((b.skillMatches || 0) !== (a.skillMatches || 0)) {
      return (b.skillMatches || 0) - (a.skillMatches || 0);
    }
  ...
  const webDevWithRatings = sortByRating(await enrichWithRating(webDevCandidates));
  const mlWithRatings = sortByRating(await enrichWithRating(mlCandidates));
  const otherWithRatings = sortByRating(await enrichWithRating(otherCandidates));
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        {
          forYou: forYouWithRatings.slice(0, 5),
          webDev: webDevWithRatings,
          ml: mlWithRatings,
          others: otherWithRatings,
        },
  ...
  const message = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9f9f9; border-radius: 10px;">
      <div style="text-align: center; margin-bottom: 20px;">
        <h2 style="color: #e6c233; margin: 0;">📹 Video Call Request</h2>
      </div>
      <div style="background-color: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <p style="font-size: 16px; color: #333; margin: 0 0 15px 0;">
          <strong>${req.user.name}</strong> has requested a video call meeting:
        </p>
        ...
        <p style="color: #666; font-size: 14px; margin: 15px 0 0 0;">
          Please respond to this request through your SkillSwap application.
        </p>
      </div>
      <div style="text-align: center; color: #999; font-size: 12px;">
        <p>This is an automated message from SkillSwap</p>
      </div>
    </div>
  `;
