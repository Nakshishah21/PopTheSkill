...
export const getRatings = asyncHandler(async (req, res) => {
  ...
  const averageRating = totalRatings > 0 ? validRatings.reduce((acc, r) => acc + r.rating, 0) / totalRatings : 0;

  // Update stored user rating for consistency
  await User.findByIdAndUpdate(user._id, { rating: averageRating });
