    ...
    const getUser = async () => {
      ...
      try {
        ...
        if (ratingsResponse.data && ratingsResponse.data.data) {
          setUserRating(ratingsResponse.data.data.userRating || 0);
          setRatingCount(ratingsResponse.data.data.totalRatings || 0);
        }
      } catch (error) {
        console.log(error);
                      ...
                      {userRating !== null ? (
                        <>
                          {Array.from({ length: 5 }, (_, index) => (
                            <span key={index} style={{ color: index < userRating ? '#ffd700' : '#ddd' }}>
                              ★
                            </span>
                          ))}
                        </>
